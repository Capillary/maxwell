...
public class MaxwellKinesisProducer extends AbstractAsyncProducer {
    ...
    public MaxwellKinesisProducer(MaxwellContext context, Map<String, String> configMap) {
        super(context);
        ...
        this.partitioner =
                new MaxwellKinesisPartitioner(partitionKey, partitionColumns, partitionFallback, kinesisMd5Keys);

        String accessKey = configMap.get(ACCESS_KEY);
        String secretKey = configMap.get(SECRET_KEY);
        String streamName = configMap.get(STREAM_NAME);
        String region = configMap.get(REGION);
        String configFilePath = configMap.get(CONFIG_FILE);

        this.kinesisStream = streamName;

        Path path = Paths.get(configFilePath);
        BasicAWSCredentials awsCreds = new BasicAWSCredentials(accessKey, secretKey);
        AWSCredentialsProvider credentialProvider = new AWSStaticCredentialsProvider(awsCreds);

        logger.info("starting Maxwell kinesis producer");

        if (Files.exists(path) && Files.isRegularFile(path)) {
            logger.info("getting kinesis properties from file {}", configFilePath);
            ...
            logger.info("no kinesis properties file provided so starting with producer default properties");
